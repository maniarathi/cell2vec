...
class LoomDataset(GeneExpressionDataset):
    ...
    def __init__(self, filename, save_path='data/', url=None):
        ...
        data, batch_indices, ethnicity_labels, organ_labels, labels, gene_names, cell_types = \
        ...
        super().__init__(X, local_means, local_vars, batch_indices, labels,
                         gene_names=gene_names, cell_types=cell_types, ethnicity_labels=ethnicity_labels,
    ...
    def preprocess(self):
        ...
        if 'ethnicity_label' in ds.ca:
            print("Extracting ethnicity information from dataset")
            ethnicity_labels = ds.ca['ethnicity_label']
            ethnicity_labels = np.reshape(ethnicity_labels, (ethnicity_labels.shape[0], 1))[select]

        if 'ClusterID' in ds.ca:
            print("Extracting cluster information from dataset")
